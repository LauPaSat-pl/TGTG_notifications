...
async def main() -> None:
	...
	tgtg_client: TgtgClient = TgtgClient(access_token=access_token, refresh_token=refresh_token, user_id=user_id, cookie=cookie)
	bot: Bot = Bot(telegram_bot_id)
	for item in tgtg_client.get_items():
		items_available: int = item['items_available']
		if items_available > 0:
			await bot.send_message(chat_id=telegram_chat_id, text=f"There are {items_available} bags")
